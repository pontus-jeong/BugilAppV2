...
import bugil.bada.bugilapp.tool.ProcessTask;
    ...
    static void updateLargeAppWidget(Context mContext, AppWidgetManager appWidgetManager, int appWidgetId, boolean ifNotUpdate) {
        ...
        if (mData.isBlankDay) {
            ...
            if (Tools.isOnline(mContext)) {
                // Only Wifi && Not Wifi
                if (new Preference(mContext).getBoolean("updateWiFi", true) && !Tools.isWifi(mContext)) {
                    mViews.setTextViewText(R.id.mBreakfast, mContext.getString(R.string.widget_no_data));
                    mViews.setTextViewText(R.id.mLunch, mContext.getString(R.string.widget_no_data));
            ...
            // 데이터 있음
            String mBreakfast = mData.Breakfast;
            String mLunch = mData.Lunch;
            String mDinner = mData.Dinner;

            if (BapTool.mStringCheck(mBreakfast)) {
                mBreakfast = mContext.getString(R.string.no_data_breakfast);
            } else {
                mBreakfast = BapTool.replaceString(mBreakfast);
            }
            ...
            mViews.setTextViewText(R.id.mBreakfast, mLunch);
